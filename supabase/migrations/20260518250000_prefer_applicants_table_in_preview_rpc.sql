-- Prefer public.applicants over stale projects.applicant_details JSON for preview/signing.
...
AS $$
  SELECT jsonb_build_object(
    ...
    'applicant_details', CASE
      WHEN EXISTS (SELECT 1 FROM public.applicants a WHERE a.project_id = p.id)
        THEN COALESCE(
