-- Allow appointed architect to record owner signature fields when completing
-- the owner step with the owner's DSC (on-behalf UI flow).
-- Scope: Architect appointment applications only; owner must not have signed yet.
...
COMMENT ON FUNCTION public.update_application_for_signing(uuid, uuid, text, timestamptz, uuid, timestamptz, uuid) IS
  'Patch application signatures: owner fields require project owner, or appointed architect on Architect appointment before owner has signed (on-behalf DSC). Second-party fields require appointed architect/consultant after owner signed.';
