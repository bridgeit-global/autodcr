...
class DSCService {
    ...
    async login(pin = '', slotIndex = 0) {
        try {
            ...
            try {
                if (pin) {
                    // Force fresh authentication so stale logged-in sessions
                    // cannot make an incorrect PIN appear valid.
                    try {
                        this.module.C_Logout(this.session);
                        this.isLoggedIn = false;
                    } catch (logoutError) {
                        // Ignore logout errors; token may already be logged out.
                    }
                ...
                // Already-logged-in must not be treated as success for PIN validation,
                // otherwise wrong PIN can pass when a prior session is still authenticated.
                if (errorMessage.includes('CKR_USER_ALREADY_LOGGED_IN')) {
                    this.isLoggedIn = false;
                    return {
                        success: false,
                        error: 'Unable to verify PIN in current session. Please reinsert DSC token and try again.'
                    };
                }
    ...
    async findPrivateKey(pin = '', certIndex = 0, slotIndex = 0) {
        try {
            ...
            // Always force fresh PIN authentication when PIN is provided.
            if (pin) {
                const loginResult = await this.login(pin, slotIndex);
                if (!loginResult.success) {
                    throw new Error(loginResult.error || 'PIN authentication failed');
                }
            }
    ...
    async signData(data, pin = '') {
        try {
            ...
            if (pin) {
                // Force a fresh login for every sign attempt with PIN.
